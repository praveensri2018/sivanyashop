...
async function updateCartItemQty(cartItemId, qty, price = null) {
  const res = await query(
    `UPDATE dbo.CartItems SET Qty = @qty, Price = COALESCE(@price, Price), CreatedAt = CreatedAt OUTPUT INSERTED.* WHERE Id = @id`,
    {
...
async function getCartWithItems(userId) {
  const sqlText = `
    ...
    ISNULL(img.ImageUrl, p.ImagePath) AS ImagePath,  -- use single image
    v.VariantName,
    v.SKU,
    v.Attributes
FROM dbo.Carts c
...
async function deleteCartItem(userId, cartItemId) {
  // ensure the cart belongs to this user before deleting
  const res = await query(
    `DELETE ci
